...
#        torch.cuda.synchronize()
        ...
        for i in range(self.pre_epoch):
            ...
            for i, data in enumerate(trainloader, 0):
                ...
                one_hot = torch.zeros(inputs.shape[0], 11).cuda().scatter_(1, labels, 1)
                weight_matrix = torch.Tensor(11,inputs.shape[0]).cuda()
                weight_final = weight_matrix.copy_(sample_weight).permute(1,0)
                
                loss = self.criterion(outputs, one_hot.float(), weight_final)
                loss.backward()
                self.pre_optim.step()
            ...
            if (100 * correct.item() / total) > 95 and (100 * correct.item() / total) < 96:
                break
...
#        torch.cuda.synchronize()
        ...
        print(time_train)
        torch.save(self.net.state_dict(), 'E:\DL\JiangXinwei\python\weighted vovnet with self attention\pre_model.pth')
